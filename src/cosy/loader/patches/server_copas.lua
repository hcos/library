...
local client = function(sock,protocol)
  ...
  self.sock_send = function(_,...)
    return copas.send(sock,...)
  end

  self.sock_receive = function(_,...)
    return copas.receive(sock,...)
  end

  self.sock_close = function(_)
    sock:shutdown()
    sock:close()
  end
...
local listen = function(opts)
  ...
  copas.addserver(
    ...
    function(sock)
      ...
      repeat
        ...
        local line = copas.receive(sock,'*l')
        if line then
  ...
  self.close = function(_,keep_clients)
    ...
    if not keep_clients then
      for _,lclients in pairs(clients) do
        for lclient in pairs(lclients) do
          lclient:close()
        end
      end
    end
